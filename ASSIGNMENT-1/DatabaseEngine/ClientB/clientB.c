...
{ 
    ...
    while(1) {
        ...
        // Message Type = 3: Server Sending message to client
        buf.msg_type = 1; 
        ...
        if (msgrcv(msgid, &buf, sizeof buf.msg_text, 3, 0) == -1) 
        {
            perror("msgrcv");
            exit(1);
        }
        ...
        printf("Received by B: \"%s\"\nTime by B(in microsecond): %lu\n", buf.msg_text, end.tv_usec - start.tv_usec);  
    }
